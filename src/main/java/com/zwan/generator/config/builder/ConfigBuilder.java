package com.zwan.generator.config.builder;

import com.zwan.generator.InjectionConfig;
import com.zwan.generator.annotation.DbType;
import com.zwan.generator.config.*;
import com.zwan.generator.config.po.TableField;
import com.zwan.generator.config.po.TableFill;
import com.zwan.generator.config.po.TableInfo;
import com.zwan.generator.config.querys.H2Query;
...
public class ConfigBuilder {
    ...
    private void checkImportPackages(TableInfo tableInfo) {
        if (StringUtils.isNotEmpty(strategyConfig.getSuperEntityClass())) {
            ...
            // 无父类开启 AR 模式
            //tableInfo.getImportPackages().add(com.baomidou.mybatisplus.extension.activerecord.Model.class.getCanonicalName());  // modified by wan
        }
        if (null != globalConfig.getIdType()) {
            // 指定需要 IdType 场景
            tableInfo.getImportPackages().add(com.zwan.generator.annotation.IdType.class.getCanonicalName());
            tableInfo.getImportPackages().add(com.zwan.generator.annotation.TableId.class.getCanonicalName());
        }
        if (StringUtils.isNotEmpty(strategyConfig.getVersionFieldName())) {
            tableInfo.getFields().forEach(f -> {
                if (strategyConfig.getVersionFieldName().equals(f.getName())) {
                    tableInfo.getImportPackages().add(com.zwan.generator.annotation.Version.class.getCanonicalName());
                }
            });
        }
    }
